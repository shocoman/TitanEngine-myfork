...
std::unordered_map<ULONG_PTR, MemoryBreakpointPageData> MemoryBreakpointPages;
...
DWORD GetPageProtectionForMemoryBreakpoint(const MemoryBreakpointPageData & page)
{
    ...
    //  we cannot rely on "PAGE_EXECUTE_*" page protections for BPs on execution
    //  and should use PAGE_GUARD (or PAGE_NOACCESS) instead:
    ...
    {
        // use GUARD_PAGE unless the original page protection was PAGE_NOACCESS (incompatible with PAGE_GUARD)
...
bool IsMemoryAccessAllowed(DWORD memProtect, ULONG_PTR accessType /*0 (READ), 1 (WRITE), or 8 (EXECUTE)*/)
{
    ...
    case PAGE_EXECUTE_WRITECOPY:
        return true;
    case PAGE_READONLY:
        return isRead || (!IsDepEnabled() && isExecute);
    case PAGE_READWRITE:
    case PAGE_WRITECOPY:
        return isRead || isWrite || (!IsDepEnabled() && isExecute);
    default:
    case PAGE_NOACCESS:
        return false;
    }
