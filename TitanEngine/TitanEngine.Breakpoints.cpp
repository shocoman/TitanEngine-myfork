...
__declspec(dllexport) bool TITCALL SetMemoryBPXEx(ULONG_PTR MemoryStart, SIZE_T SizeOfMemory, DWORD BreakPointType, bool RestoreOnHit, LPVOID bpxCallBack)
{
    ...
    MemoryBreakpointPageData pageData;
    ...
    for(ULONG_PTR page = pageStart; page < pageEnd; page += TITANENGINE_PAGESIZE)
    {
        ...
        // Get a proper MemBp page protection and apply it
        pageData.newProtect = GetPageProtectionForMemoryBreakpoint(pageData);
        DWORD oldProtect;
        bool isOk = VirtualProtectEx(dbgProcessInformation.hProcess, (LPVOID)page, TITANENGINE_PAGESIZE, pageData.newProtect, &oldProtect);
...
__declspec(dllexport) bool TITCALL RemoveMemoryBPX(ULONG_PTR MemoryStart, SIZE_T SizeOfMemory)
{
    ...
    SizeOfMemory = BreakPointBuffer.at(nFoundBp).BreakPointSize; // ignore passed size, x64dbg might lie
    ...
    for(ULONG_PTR pageAddr = pageStart; pageAddr < pageEnd; pageAddr += TITANENGINE_PAGESIZE)
    {
        ...
        DWORD newProtect;
        bool noMoreBps = 0 == (pageData.accessBps + pageData.readBps + pageData.writeBps + pageData.executeBps);
        if(noMoreBps)
        {
