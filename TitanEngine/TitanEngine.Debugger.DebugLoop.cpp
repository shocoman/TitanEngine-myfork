    ...
    while(!BreakDBG) //actual debug loop
    {
        ...
        case EXCEPTION_DEBUG_EVENT:
        {
            ...
            case STATUS_ACCESS_VIOLATION:
            {
                ...
                // 3) If all are true (i.e. the BP is ours):
                ...
                if(!bFoundBreakPoint)
                {
                    // There were no BPs at the access address.
                ...
                else
                {
                    ...
                    MemoryBreakpointPageData pageData = hitPage->second;
                    ...
                    {
                        // Part 3.
                        // This was indeed our breakpoint, and of the right type, too. We can call the user callback.
                        bCallUserCallback = true;

                        if(!foundBreakPoint.MemoryBpxRestoreOnHit)
                        {
                            // BP was singleshoot and should be removed
                ...
                // Part 4. When required, reactivate our BP
                //
                // At this point, if we want to restore the breakpoint, we should temporarily put the original
                // protection back. The problem is that the original protection may not allow us to do execution
                ...
                // single-step again, and so on). So if we are sure that resetting the BP is not a good idea,
                ...
                {
                    // Mini Plan:
                    // 1) Set a protection that would allow us to normally execute the instruction that caused this exception
                    ...
                    if(bCallUserCallback)
                    {
                        ...
                        myCustomHandler((void*)foundBreakPoint.BreakPointAddress);
                    }
                    ...
                    ResetMemBpxCallback = [currentPageAddr]
                    {
                        ...
                        if(hitPage != MemoryBreakpointPages.end())
                        {
                            // BP still exists OR it's been removed and a new one added
                    ...
                    // Use the trap flag to schedule the page protection restoration on the next single-step
                    synchronizedStep = true;
                    hActiveThread = EngineOpenThread(THREAD_GETSETSUSPEND, false, DBGEvent.dwThreadId);
                    CONTEXT myDBGContext;
                    myDBGContext.ContextFlags = ContextControlFlags;
